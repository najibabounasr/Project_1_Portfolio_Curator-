...
The user will plug in their user information via the comand line interface, and have their current medical insurance cost information lined out. The portfolio currator will serve to provide the user with a portfolio generating returns that can pay for their entire medical insurance bill, within a given timeframe. The porrtfolio currator will pull in from a number of possible investable securities. The pool of possible investments will be made up of top performers in the S&P 500, and NASDAQ exchanges. Data will be pulled in via the Alpaca API, and medical insurance data from a seperate DataFrame.
